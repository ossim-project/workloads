#!/usr/bin/env python3
"""Analyze smp_pingpong JSONL result logs.

Usage:
    analyze_smp_pingpong.py /workspace/ossim/logs/smp_pingpong

The input directory is expected to contain *.log files with JSON objects emitted
by workloads/microbench/smp_pingpong/smp_pingpong. Non-JSON lines are ignored so
...
def metrics_for(rows: list[dict[str, Any]]) -> dict[str, list[float]]:
    ...
    for row in rows:
        ...
        metrics["iters"].append(float(row.get("iters", math.nan)))
...
def print_text(result: dict[str, Any]) -> None:
    ...
    if not files:
        ...
        return
    print(f"# smp_pingpong analysis: {result['log_dir']}\n")
    rows = []
    for name, data in files.items():
        s = data.get("summary", {})
        first = data["rows"][0] if data.get("rows") else {}
        if not s:
        ...
        rows.append([
            ...
            str(s["lat_mean_ns"]["n"]),
            str(first.get("mode", "?")),
            f"{first.get('cpu_a', '?')}->{first.get('cpu_b', '?')}",
            fmt(s["iters"]["median"], 0),
    ...
    print(markdown_table([
        "setup", "n", "mode", "cpus", "iters", "mean ns", "p50 ns", "p90 ns",
        "p99 ns", "max ns", "mean CV", "zero", ">10ms", ">100ms",
    ], rows))
    ...
    if base_name:
        ...
        if base and not math.isnan(base):
            ...
            print(f"\n## Median p50 latency relative to {base_name}\n")
            print(markdown_table(["setup", "p50 latency ratio", "median p99 ns"], ratio_rows))

    print("\n## Raw median p50 round-trip latency (ns)\n")
    for name, data in readable.items():
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Analyze smp_pingpong JSONL logs")
    parser.add_argument("log_dir", type=Path, help="Directory containing smp_pingpong *.log files")
